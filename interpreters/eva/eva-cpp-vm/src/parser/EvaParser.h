...
class Tokenizer {
 ...
 private:
   ...
   */
  // clang-format off
  static constexpr size_t LEX_RULES_COUNT = 6;
...
std::array<LexRule, Tokenizer::LEX_RULES_COUNT> Tokenizer::lexRules_ = {{
  ...
  {std::regex(R"(^\d+)"), &_lexRule5},
  {std::regex(R"(^[\w\-+*=<>/]+)"), &_lexRule6}
}};
std::map<TokenizerState, std::vector<size_t>> Tokenizer::lexRulesByStartConditions_ =  {{TokenizerState::INITIAL, {0, 1, 2, 3, 4, 5}}};
// clang-format on
